...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload DX before expiry.${plain}"
    return 0
}
...
config_after_install() {
    local existing_hasDefaultCredential=$(${DX_folder}/DX setting -show true | grep -Eo 'hasDefaultCredential: .+' | awk '{print $2}')
    ...
    local existing_cert=$(${DX_folder}/DX setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            local db_label="SQLite (/etc/DX/DX.db)"
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        DX_env_file="/etc/default/DX"
                        ;;
                    arch | manjaro | parch | alpine)
                        DX_env_file="/etc/conf.d/DX"
                        ;;
                    *)
                        DX_env_file="/etc/sysconfig/DX"
                        ;;
                esac
                ...
                while [[ -z "$DX_dsn" ]]; do
                    ...
                    if [[ "$pg_mode" == "2" ]]; then
                        while [[ -z "$DX_dsn" ]]; do
                            read -rp "Enter PostgreSQL DSN (postgres://user:pass@host:port/dbname?sslmode=disable): " DX_dsn
                            DX_dsn="${DX_dsn// /}"
                        done
                        ...
                        local pg_cred_file
                        pg_cred_file=$(mktemp 2> /dev/null) || pg_cred_file=$(mktemp -t DX-pg-creds.XXXXXXXX)
                        if [[ -z "${pg_cred_file}" ]]; then
                            echo -e "${red}Failed to create temporary credentials file.${plain}"
                            DX_dsn=""
                            continue
                        fi
                        if DX_dsn=$(PG_CRED_FILE="${pg_cred_file}" install_postgres_local); then
                ...
                if [[ -n "$DX_dsn" ]]; then
                    install -d -m 755 "$(dirname "$DX_env_file")"
                    umask 077
                    cat > "$DX_env_file" << EOF
DX_DB_TYPE=postgres
DX_DB_DSN=${DX_dsn}
EOF
                    chmod 600 "$DX_env_file"
                    umask 022
                    export DX_DB_TYPE=postgres
                    export DX_DB_DSN="${DX_dsn}"
            ...
            ${DX_folder}/DX setting -username "${config_username}" -password "${config_password}" -port "${config_port}" -webBasePath "${config_webBasePath}"
            ...
            local config_apiToken=$(${DX_folder}/DX setting -getApiToken true | grep -Eo 'apiToken: .+' | awk '{print $2}')
            ...
            if [[ "$db_choice" == "2" && "$pg_local_installed" == "1" ]]; then
                ...
                echo -e "${green}DSN:        ${DX_dsn}${plain}"
                echo -e "${green}Env file:   ${DX_env_file}${plain}"
                echo -e "${green}-------------------------------------------${plain}"
                ...
                echo -e "${green}═══════════════════════════════════════════${plain}"
                echo -e "${yellow}⚠ The panel reads these credentials from ${DX_env_file}.${plain}"
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            ${DX_folder}/DX setting -username "${config_username}" -password "${config_password}"
        ...
        # Properly detect empty cert by checking if cert: line exists and has content after it
        existing_cert=$(${DX_folder}/DX setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
        if [[ -z "$existing_cert" ]]; then
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${green}     SSL Certificate Setup (RECOMMENDED)   ${plain}"
...
install_DX() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/MHSanaei/DX/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "https://api.github.com/repos/MHSanaei/DX/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
                echo -e "${red}Failed to fetch DX version, it may be due to GitHub API restrictions, please try it later${plain}"
                exit 1
            fi
        fi
        echo -e "Got DX latest version: ${tag_version}, beginning the installation..."
        curl -4fLRo ${DX_folder}-linux-$(arch).tar.gz https://github.com/MHSanaei/DX/releases/download/${tag_version}/DX-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading DX failed, please be sure that your server can access GitHub ${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        url="https://github.com/MHSanaei/DX/releases/download/${tag_version}/DX-linux-$(arch).tar.gz"
        echo -e "Beginning to install DX $1"
        curl -4fLRo ${DX_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download DX $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    curl -4fLRo /usr/bin/DX-temp https://raw.githubusercontent.com/MHSanaei/DX/main/DX.sh
    if [[ $? -ne 0 ]]; then
        echo -e "${red}Failed to download DX.sh${plain}"
        exit 1
    fi

    # Stop DX service and remove old resources
    if [[ -e ${DX_folder}/ ]]; then
        if [[ $release == "alpine" ]]; then
            rc-service DX stop
        else
    ...
    tar zxvf DX-linux-$(arch).tar.gz
    rm DX-linux-$(arch).tar.gz -f
    ...
    chmod +x DX bin/xray-linux-$(arch)
    ...
    # Update DX cli and se set permission
    mv -f /usr/bin/DX-temp /usr/bin/DX
    chmod +x /usr/bin/DX
    mkdir -p /var/log/DX
    config_after_install
    ...
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "DX/DX.db" "/etc/.gitignore"; then
                echo "" >> "/etc/.gitignore"
                echo "DX/DX.db" >> "/etc/.gitignore"
                echo -e "${green}Added DX.db to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "DX/DX.db" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added DX.db for etckeeper${plain}"
        fi
    fi

    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/DX https://raw.githubusercontent.com/MHSanaei/DX/main/DX.rc
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download DX.rc${plain}"
            exit 1
        fi
        chmod +x /etc/init.d/DX
        rc-update add DX
        rc-service DX start
    else
        ...
        if [ -f "DX.service" ]; then
            echo -e "${green}Found DX.service in extracted files, installing...${plain}"
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "DX.service.debian" ]; then
                        echo -e "${green}Found DX.service.debian in extracted files, installing...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "DX.service.arch" ]; then
                        echo -e "${green}Found DX.service.arch in extracted files, installing...${plain}"
                ...
                *)
                    if [ -f "DX.service.rhel" ]; then
                        echo -e "${green}Found DX.service.rhel in extracted files, installing...${plain}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${DX_service}/DX.service https://raw.githubusercontent.com/MHSanaei/DX/main/DX.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${DX_service}/DX.service https://raw.githubusercontent.com/MHSanaei/DX/main/DX.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${DX_service}/DX.service https://raw.githubusercontent.com/MHSanaei/DX/main/DX.service.rhel > /dev/null 2>&1
                    ;;
            esac
        ...
        if [ "$service_installed" = true ]; then
            ...
            echo -e "${red}Failed to install DX.service file${plain}"
            exit 1
        fi
    fi

    echo -e "${green}DX ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
