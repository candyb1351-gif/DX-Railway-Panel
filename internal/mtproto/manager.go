...
type Manager struct {
	...
	// processes (survivors of a previous DX run) has already run.
	swept bool
}
...
// sweepOrphansLocked kills mtg processes left running by a previous DX run,
// exactly once per process lifetime and before any of our own mtg are started.
// Because DX owns every mtg process, anything alive at this point is an orphan
...
// — which DX always emits and scrapes for traffic (see scrapeTraffic).
