...
// for the DX panel using GORM with SQLite or PostgreSQL.
package database
...
import (
	...
	"github.com/mhsanaei/DX/v3/internal/database/model"
	"github.com/mhsanaei/DX/v3/internal/util/crypto"
	"github.com/mhsanaei/DX/v3/internal/util/random"
...
// When DX_DB_TYPE=postgres, dbPath is ignored and DX_DB_DSN is used instead.
func InitDB(dbPath string) error {
	...
	case "postgres":
		dsn := config.GetDBDSN()
		if dsn == "" {
			return errors.New("DX_DB_TYPE=postgres but DX_DB_DSN is empty")
		}
	...
	switch config.GetDBKind() {
	case "postgres":
		maxOpen = envInt("DX_DB_MAX_OPEN_CONNS", 25)
		maxIdle = envInt("DX_DB_MAX_IDLE_CONNS", 25)
	default:
		maxOpen = envInt("DX_DB_MAX_OPEN_CONNS", 8)
		maxIdle = envInt("DX_DB_MAX_IDLE_CONNS", 4)
	}
