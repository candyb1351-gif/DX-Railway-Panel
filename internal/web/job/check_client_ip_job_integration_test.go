...
import (
	...
	DXlogger "github.com/mhsanaei/DX/v3/internal/logger"
	"github.com/op/go-logging"
)

// DX logger must be initialised once before any code path that can
...
func setupIntegrationDB(t *testing.T) {
	...
	loggerInitOnce.Do(func() {
		DXlogger.InitLogger(logging.ERROR)
	})

	dbDir := t.TempDir()
	logDir := t.TempDir()

	t.Setenv("DX_DB_FOLDER", dbDir)
	t.Setenv("DX_LOG_FOLDER", logDir)
	...
	if err := database.InitDB(filepath.Join(dbDir, "DX.db")); err != nil {
...
func TestRun_DisabledFail2BanSkipsProbeAndBanLog(t *testing.T) {
	...
	t.Setenv("DX_ENABLE_FAIL2BAN", "false")
...
func TestRun_CollectsIpsWithoutLimit(t *testing.T) {
	setupIntegrationDB(t)
	t.Setenv("DX_ENABLE_FAIL2BAN", "true")
...
func readIpLimitLogPath() string {
	folder := os.Getenv("DX_LOG_FOLDER")
	if folder == "" {
