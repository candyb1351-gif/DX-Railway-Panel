...
import (
	...
	"github.com/mhsanaei/DX/v3/internal/xray"
...
func (s *ServerService) GetLogs(count string, level string, syslog string) []string {
	...
	if syslog == "true" {
		...
		cmd := exec.Command("journalctl", "-u", "DX", "--no-pager", "-n", strconv.Itoa(countInt), "-p", level)
		var out bytes.Buffer
		cmd.Stdout = &out
		err = cmd.Run()
		if err != nil {
			return []string{"Failed to run journalctl command! Make sure systemd is available and DX service is registered."}
		}
...
func (s *ServerService) GetMigration() ([]byte, string, error) {
	if database.IsPostgres() {
		tmp, err := os.CreateTemp("", "DX-migration-*.db")
		if err != nil {
			return nil, "", err
		}
		tmpPath := tmp.Name()
...
func (s *ServerService) importPostgresDB(file multipart.File) error {
	...
	tempFile, err := os.CreateTemp("", "DX-pg-restore-*.dump")
	if err != nil {
