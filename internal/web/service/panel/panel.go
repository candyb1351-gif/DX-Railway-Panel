...
const (
	panelUpdaterURL      = "https://raw.githubusercontent.com/MHSanaei/DX/main/update.sh"
...
// GetUpdateInfo checks GitHub for the latest DX release.
...
func (s *PanelService) StartUpdate() error {
	...
	if systemdRun, err := exec.LookPath("systemd-run"); err == nil {
		unitName := fmt.Sprintf("DX-web-update-%d", time.Now().Unix())
		cmd := exec.Command(systemdRun,
			"--unit", unitName,
			"--setenv", "DX_MAIN_FOLDER="+mainFolder,
			"--setenv", "DX_SERVICE="+serviceFolder,
	...
	cmd.Env = append(os.Environ(),
		"DX_MAIN_FOLDER="+mainFolder,
		"DX_SERVICE="+serviceFolder,
	)
...
func downloadPanelUpdater() (string, error) {
	...
	file, err := os.CreateTemp("", "DX-update-*.sh")
	if err != nil {
		return "", err
	}
	path := file.Name()
...
func fetchLatestPanelVersion() (string, error) {
	...
	resp, err := client.Get("https://api.github.com/repos/MHSanaei/DX/releases/latest")
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
...
func resolveUpdateFolders() (string, string) {
	mainFolder := os.Getenv("DX_MAIN_FOLDER")
	if mainFolder == "" {
	...
	if mainFolder == "" {
		mainFolder = "/usr/local/DX"
	}

	serviceFolder := os.Getenv("DX_SERVICE")
	if serviceFolder == "" {
