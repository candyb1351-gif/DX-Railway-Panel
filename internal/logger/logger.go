// Package logger provides logging functionality for the DX panel with
...
const (
	...
	logFileName      = "DX.log"            // Log file name
...
func InitLogger(level logging.Level) {
	newLogger := logging.MustGetLogger("DX")
	...
	if consoleBackend := initDefaultBackend(); consoleBackend != nil {
		...
		leveledBackend.SetLevel(level, "DX")
	...
	if fileBackend := initFileBackend(); fileBackend != nil {
		...
		leveledBackend.SetLevel(logging.DEBUG, "DX")
		backends = append(backends, leveledBackend)
	}
