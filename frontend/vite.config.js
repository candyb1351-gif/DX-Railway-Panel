...
function resolveDBPath() {
  const envFolder = process.env.DX_DB_FOLDER;
  if (envFolder) {
    const abs = path.isAbsolute(envFolder)
      ? envFolder
      : path.resolve(__dirname, '..', envFolder);
    return path.join(abs, 'DX.db');
  }
  const repoSubDB = path.resolve(__dirname, '..', 'DX', 'DX.db');
  if (fs.existsSync(repoSubDB)) return repoSubDB;
  const repoDB = path.resolve(__dirname, '..', 'DX.db');
