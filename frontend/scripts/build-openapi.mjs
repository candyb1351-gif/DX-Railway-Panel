...
function buildSpec() {
  ...
  return {
    ...
    info: {
      ...
      description:
        'Programmatic interface to a DX panel. Authenticate either by logging in (cookie) or with an API token from Settings → Security → API Token (Bearer). All endpoints under /panel/api/* honour both modes — an API token is a full-admin credential, so treat it like the panel password.',
    },
    servers: [
