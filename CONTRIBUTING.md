...
Thanks for taking the time to contribute to DX. This guide gets a development panel running locally and explains the conventions the project follows so changes land cleanly.
...
```bash
git clone https://github.com/MHSanaei/DX.git
cd DX
...
`.env.example` ships with defaults that keep the database, logs, and xray binary inside the local `DX/` folder so nothing escapes the project directory:
...
DX_LOG_FOLDER=DX
DX_BIN_FOLDER=DX
DX_INIT_WEB_BASE_PATH=/
```

Drop the xray binary (`xray-windows-amd64.exe` on Windows, `xray-linux-amd64` on Linux, etc.) plus the matching `geoip.dat` and `geosite.dat` files into `DX/`. The easiest source is a [released Xray-core build](https://github.com/XTLS/Xray-core/releases). On Windows, `wintun.dll` is also required for testing TUN inbounds.
...
The repo checks in two VS Code launch profiles in `.vscode/launch.json`: **Run DX (Debug)** for the default SQLite setup, and **Run DX (Postgres)** which points `DX_DB_TYPE`/`DX_DB_DSN` at a local PostgreSQL. The Postgres profile also prepends the PostgreSQL `bin` to `PATH` so the panel can find `pg_dump`/`pg_restore` (the `postgresql-client` tools used for DB backup/restore) — adjust the DSN and that path to your machine:
  ...
  "configurations": [
    ...
    },
    {
      ...
      "env": {
        ...
        "DX_BIN_FOLDER": "DX",
        "DX_DB_TYPE": "postgres",
        "DX_DB_DSN": "postgres://DX:DXpass@127.0.0.1:5432/DX?sslmode=disable",
...
| `DX/` | **Runtime data** — db, logs, xray binary, geo files (gitignored) |
...
|----------|---------|---------|
| `DX_DEBUG` | `false` | Verbose logs + Gin debug mode + serve `/assets` from disk |
| `DX_LOG_LEVEL` | `info` | `debug` / `info` / `notice` / `warning` / `error` |
| `DX_DB_FOLDER` | platform default | Where `DX.db` lives |
| `DX_LOG_FOLDER` | platform default | Where `DX.log` lives |
| `DX_BIN_FOLDER` | `bin` | Where the xray binary, geo files, and xray `config.json` live |
| `DX_INIT_WEB_BASE_PATH` | `/` | The initial URI path for the web panel |
| `DX_DB_TYPE` | `sqlite` | Set to `postgres` to use PostgreSQL via `DX_DB_DSN` |
| `DX_DB_DSN` | — | PostgreSQL DSN when `DX_DB_TYPE=postgres` |

## Issues

- Bug reports and feature requests: [GitHub Issues](https://github.com/MHSanaei/DX/issues)

Before filing a bug, include the OS, Go version, panel version (`/panel/api/server/status` or the dashboard footer), and the relevant excerpt from `DX/DX.log`.
