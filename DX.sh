...
if [[ -f /.dockerenv ]] || [[ "${DX_IN_DOCKER}" == "true" ]]; then
    running_in_docker="true"
fi

# Declare Variables
if [[ "${running_in_docker}" == "true" ]]; then
    DX_folder="${DX_MAIN_FOLDER:=/app}"
else
    DX_folder="${DX_MAIN_FOLDER:=/usr/local/DX}"
fi
DX_service="${DX_SERVICE:=/etc/systemd/system}"
log_folder="${DX_LOG_FOLDER:=/var/log/DX}"
mkdir -p "${log_folder}"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/DX/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "This function will update all DX components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/DX/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/DX https://raw.githubusercontent.com/MHSanaei/DX/main/DX.sh
    chmod +x ${DX_folder}/DX.sh
    chmod +x /usr/bin/DX
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/mhsanaei/DX/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        systemctl stop DX
        systemctl disable DX
        rm ${DX_service}/DX.service -f
    ...
    rm /etc/DX/ -rf
    rm ${DX_folder}/ -rf
    rm -f "$(DX_env_file_path)"

    echo ""
    echo -e "Uninstalled Successfully.\n"
    echo "If you need to install this panel again, you can use below command:"
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/mhsanaei/DX/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${DX_folder}/DX setting -username "${config_account}" -password "${config_password}" > /dev/null 2>&1
    else
        ${DX_folder}/DX setting -username "${config_account}" -password "${config_password}" -resetTwoFactor=true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the DX panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${DX_folder}/DX setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${DX_folder}/DX setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${DX_folder}/DX setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local db_env_file
    db_env_file="$(DX_env_file_path)"
    if [[ -r "$db_env_file" ]] && grep -q '^DX_DB_TYPE=postgres' "$db_env_file"; then
        local dsn
        dsn="$(grep -E '^DX_DB_DSN=' "$db_env_file" | head -1 | cut -d= -f2-)"
        local dsn_safe
        dsn_safe="$(echo "$dsn" | sed -E 's|(://[^:/@]+:)[^@]+@|\1****@|')"
        echo -e "${green}Database: PostgreSQL — ${dsn_safe}${plain}"
    else
        echo -e "${green}Database: SQLite (/etc/DX/DX.db)${plain}"
    fi
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${DX_folder}/DX setting -port ${port}
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "DX and xray stopped successfully"
        else
...
restart() {
    if [[ "${running_in_docker}" == "true" ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "DX and xray Restarted successfully"
        else
...
status() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service DX status
    else
        systemctl status DX -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-update add DX default
    else
        systemctl enable DX
    fi
    if [[ $? == 0 ]]; then
        LOGI "DX Set to boot automatically on startup successfully"
    else
        LOGE "DX Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "DX Autostart Cancelled successfully"
    else
        LOGE "DX Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u DX -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-DX.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-DX.conf | tr -d '#')
        ...
        rm /etc/sysctl.d/99-bbr-DX.conf
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        sysctl -p /etc/sysctl.d/99-bbr-DX.conf
    else
...
update_shell() {
    curl -fLRo /usr/bin/DX -z /usr/bin/DX https://github.com/MHSanaei/DX/raw/main/DX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
DX_pid() {
    ps -ef 2> /dev/null | grep -F "${DX_folder}/DX" | grep -v grep | awk 'NR==1 {print $1}'
}
...
check_status() {
    ...
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service DX status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status DX | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F 'DX' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled DX)
...
show_mtproto_status() {
    local cfg_dir="${DX_folder}/bin/mtproto"
    local cfgs=()
    if [[ -d "${cfg_dir}" ]]; then
...
update_geofiles() {
    ...
    for dat in "${dat_files[@]}"; do
        ...
        remote_file="${dat%%_*}"
        curl -fLRo ${DX_folder}/bin/${dat}.dat -z ${DX_folder}/bin/${dat}.dat \
...
ssl_cert_issue_main() {
    ...
    case "$choice" in
        ...
        4)
            ...
            local panel_cert=$(${DX_folder}/DX setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${DX_folder}/DX setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart DX 2>/dev/null || rc-service DX restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="DX restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}DX restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; DX restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; DX restart"
                reloadCmd="systemctl reload nginx ; DX restart"
                ;;
            2)
                LOGD "It's recommended to put DX restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; DX restart): " reloadCmd
...
EOF
    ...
    local panel_port
    panel_port=$(${DX_folder}/DX setting -show true 2>/dev/null | grep -Eo 'port: .+' | awk '{print $2}')
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${DX_folder}/DX setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${DX_folder}/DX setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${DX_folder}/DX setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${DX_folder}/DX setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${DX_folder}/DX setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${DX_folder}/DX setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${DX_folder}/DX setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
# PostgreSQL service management (for panels configured with DX_DB_TYPE=postgres).
...
# Installs a local PostgreSQL server and creates a dedicated DX user/database.
...
# Writes DX_DB_TYPE/DX_DB_DSN into the service env file, preserving other entries.
pg_write_env() {
    local dsn="$1" envfile
    envfile="$(DX_env_file_path)"
    install -d -m 755 "$(dirname "$envfile")"
    touch "$envfile"
    sed -i '/^DX_DB_TYPE=/d; /^DX_DB_DSN=/d' "$envfile"
    {
        echo "DX_DB_TYPE=postgres"
        echo "DX_DB_DSN=${dsn}"
    } >> "$envfile"
    chmod 600 "$envfile"
}
...
pg_install_server_action() {
    if postgresql_installed; then
        ...
        confirm "Run setup anyway (ensures the DX database/user exist)?" "n" || return 0
    fi
...
migrate_to_postgres() {
    ...
    if ! ${DX_folder}/DX migrate-db --dsn "$dsn"; then
    ...
    pg_write_env "$dsn"
    LOGI "Wrote database settings to $(DX_env_file_path) (DX_DB_TYPE=postgres)."
...
postgresql_menu() {
    echo -e "${green}\t1.${plain} ${green}Install${plain} PostgreSQL (server + client + DX db)"
...
# file using the bundled DX binary. With no arguments it dumps the installed
# panel database; an optional second argument overrides the output path.
#   DX migrateDB [file.db|file.dump] [output]
migrate_db() {
    local input="$1" output="$2"
    local default_db="/etc/DX/DX.db"
    local bin="${DX_folder}/DX"

    [[ -z "$input" ]] && input="$default_db"

    if [[ ! -x "$bin" ]]; then
        LOGE "DX binary not found at ${bin}. Is the panel installed?"
        return 1
    fi

    if ! "$bin" migrate-db -h 2>&1 | grep -q -- '-dump'; then
        LOGE "This DX build does not support .db <-> .dump conversion yet."
        LOGE "Update the panel first (DX update) to a version with 'migrate-db --dump/--restore'."
        return 1
    fi

    if [[ ! -f "$input" ]]; then
        LOGE "Input file not found: ${input}"
        echo -e "Usage: ${green}DX migrateDB [file.db|file.dump] [output]${plain}"
        return 1
    fi
    ...
    if [[ "$mode" == "dump" ]]; then
        ...
        if [[ "$output" == "$default_db" ]] && check_status > /dev/null 2>&1; then
            LOGE "Refusing to restore into the live database (${default_db}) while DX is running."
            LOGE "Stop the panel first (DX stop) or choose a different output path."
            return 1
        fi
        if [[ -f "$output" ]]; then
            confirm "Output ${output} already exists and will be overwritten. Continue?" "n" || return 0
...
|  ${blue}DX restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}DX update-all-geofiles${plain}   - Update all geo files             │
│  ${blue}DX migrateDB [file]${plain}      - Convert .db <-> .dump (SQLite)   │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}DX Panel Management Script${plain}                │
